  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !navBarBtnRef.current.contains(event.target)
      ) {
        closeNavBar();
      }
    };
    ...
    <div className="flex justify-between items-center p-3.5 px-[var(--pub-container-padding-x-mobile)] md:px-[var(--pub-container-padding-x)] xl:pt-0 xl:pb-0 fixed top-0 left-0 w-full bg-black/25 backdrop-blur-md z-50">
      <nav className="flex items-center justify-between w-[87.5%] sm:w-[92%] md:w-11/12 xl:w-max xl:gap-7 xl:justify-baseline">
        ...
        <div className="flex relative xl:hidden">
          <div
            ref={navBarBtnRef}
            className={`flex ${isNavBarOpen ? "text-[var(--pink-color)]" : "text-white"} cursor-pointer`}
          ...
          {isNavBarOpen && (
            <ul
              ref={navBarRef}
              className="flex flex-col text-right absolute top-10 md:top-11.5 right-0 bg-[var(--pink-color)] p-1.5 pl-3.5 pr-3.5 rounded-2xl rounded-tr-none"
            >
              <li className="flex monument-regular font-bold text-[14px] w-full cursor-pointer">
                <NavLink
                  className={({ isActive }) =>
                    `w-full hover:text-black ${isActive ? "text-black" : "text-white"}`
                  }
                  to="/about"
                  onClick={closeNavBar}
                >
        ...
        <div className="hidden xl:flex">
          <ul className="flex xl:flex-row xl:static p-1.5 pl-3.5 pr-3.5 rounded-2xl rounded-tr-none">
            <li className="flex monument-regular font-bold w-full cursor-pointer">
              <NavLink
                className={({ isActive }) =>
                  `w-full xl:p-4 xl:pl-9 xl:pr-9 xl:hover:text-[var(--pink-color)] ${isActive ? "text-[var(--pink-color)]" : "text-white"}`
                }
                to="/about"
              >
                ABOUT
      ...
      <NavLink
        className={({ isActive }) =>
          `flex monument-extra-bold text [1rem] hover:text-[#DB3F7A] cursor-pointer xl:pt-4 xl:pb-4 ${isActive ? "text-[var(--pink-color)]" : "text-white"}`
        }
        to="/sign-in"
      >
