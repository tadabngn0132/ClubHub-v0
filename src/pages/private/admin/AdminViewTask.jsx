...
const AdminViewTask = () => {
  ...
  const filterCurrentUserAssignee = task?.assignees?.filter(assignee => assignee.assigneeId === currentUser.id);
  const filterConfirmedAssignee = task?.assignees?.filter(assignee => assignee.evidenceUrl);
  const statusLabel = task?.status ? formatUppercaseToCapitalized(task.status) : "N/A";
  ...
  const statusClass = statusClassMap[statusLabel] || "bg-gray-700/40 text-gray-300 border-gray-600";
      ...
      <div className="mx-auto w-full max-w-5xl">
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-900/90 p-5 shadow-xl backdrop-blur md:p-6">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <h1 className="text-2xl font-bold tracking-tight text-gray-100 md:text-3xl">{task?.title}</h1>

            <div className="flex items-center gap-2 rounded-lg border border-gray-700 bg-gray-800 p-2 text-sm shadow-md">
              <Link
                to={`/admin/tasks/edit/${taskId}`}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <p className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${statusClass}`}>
              {statusLabel}
            </p>
          </div>
          ...
          <div className="mt-5 grid gap-4 text-sm text-gray-200 md:grid-cols-2">
            <p className="rounded-lg border border-gray-800 bg-gray-950/60 p-3">
              <span className="font-semibold text-gray-400">Deadline:</span> 23h59 {task?.dueDate ? formatDate(task?.dueDate) : "N/A"}
            </p>
            <p className="rounded-lg border border-gray-800 bg-gray-950/60 p-3">
              <span className="font-semibold text-gray-400">Updated At:</span> {task?.updatedAt ? formatDate(task?.updatedAt) : "N/A"}
            </p>
          </div>

          <div className="mt-4 rounded-lg border border-gray-800 bg-gray-950/60 p-4">
            <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-gray-400">Description</h2>
            <p className="whitespace-pre-line text-sm leading-6 text-gray-200">{task?.description || "No description provided."}</p>
          </div>
        </div>

        <div className="mt-6 rounded-xl border border-gray-800 bg-gray-900/90 p-5 shadow-xl md:p-6">
          <h1 className="mb-4 text-xl font-bold text-gray-100 md:text-2xl">Assignee Confirmation</h1>
          <TaskConfirmationForm taskCfData={filterCurrentUserAssignee} onSubmit={handleConfirmCompletion} />
          {filterConfirmedAssignee && filterConfirmedAssignee.length > 0 && (
            <div className="mt-6 rounded-lg border border-green-700 bg-green-900/60 p-4">
              <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-green-400">Confirmed Completion</h2>
              {filterConfirmedAssignee.map(assignee => (
                <div key={assignee.assigneeId} className="mb-4">
                  <p className="text-sm text-green-300">
                    <span className="font-semibold text-green-400">Assignee:</span> {assignee.assigneeName || "N/A"}
                  </p>
                  <p className="text-sm text-green-300">
                    <span className="font-semibold text-green-400">Comments:</span> {assignee.additionalComments || "N/A"}
                  </p>
                  {assignee.evidenceUrl && (
                    <div className="mt-2">
                      <p className="text-sm font-semibold text-green-400">Evidence:</p>
                      <img src={assignee.evidenceUrl} alt="Evidence" className="mt-1 max-h-60 rounded-md object-cover" />
                    </div>
                  )}
                </div>
              ))}
