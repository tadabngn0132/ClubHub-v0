      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <section className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-slate-300">
              #{memberApplication?.id || "N/A"} - {memberApplication?.fullname || "N/A"}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link
              ...
              className={`rounded-lg border border-emerald-500/40 bg-emerald-500/15 px-3 py-2 text-sm font-medium text-emerald-300 hover:bg-emerald-500/25 ${
                canInterview && interviewSummary.total > 0 && interviewSummary.pending === 0
                  ? ""
                  : actionDisabledClass
              }`}
              title={
        ...
        <section className="rounded-2xl border border-slate-700 bg-slate-900/65 p-4">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {timelineItems.map((item, index) => (
              <div key={item.key} className="rounded-xl border border-slate-700 bg-slate-950/40 p-3">
                ...
                <p className="mt-2 text-xs text-slate-400">{item.done ? "Completed" : "In Progress"}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="grid gap-3 md:grid-cols-4">
          <div className="rounded-xl border border-slate-700 bg-slate-900/65 p-3">
            ...
            <span className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${badgeClass(cvStatus)}`}>
              {cvStatus || "N/A"}
            </span>
          </div>
          <div className="rounded-xl border border-slate-700 bg-slate-900/65 p-3">
            <p className="text-xs uppercase text-slate-400">Interview Summary</p>
          ...
          <div className="rounded-xl border border-slate-700 bg-slate-900/65 p-3">
            ...
            <span className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${badgeClass(finalStatus)}`}>
              {finalStatus || "N/A"}
            </span>
          </div>
          <div className="rounded-xl border border-slate-700 bg-slate-900/65 p-3">
            <p className="text-xs uppercase text-slate-400">Round Gates</p>
            <p className="mt-2 text-sm text-slate-100">Interview: {canInterview ? "Open" : "Locked"}</p>
            <p className="text-sm text-slate-100">Final: {canFinalReview ? "Open" : "Locked"}</p>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-700 bg-slate-900/65 p-4">
          ...
          <div className="mb-4 flex items-center gap-4 rounded-xl border border-slate-700 bg-slate-950/40 p-3">
            ...
            <div>
              <p className="text-base font-semibold text-slate-100">{memberApplication?.fullname || "N/A"}</p>
              <p className="text-sm text-slate-300">{memberApplication?.email || "N/A"}</p>
            </div>
          </div>

          <div className="grid gap-3 md:grid-cols-2">
            <p><span className="text-slate-400">Full Name:</span> {memberApplication?.fullname || "N/A"}</p>
            <p><span className="text-slate-400">Email:</span> {memberApplication?.email || "N/A"}</p>
            <p><span className="text-slate-400">Phone:</span> {memberApplication?.phoneNumber || "N/A"}</p>
            <p><span className="text-slate-400">Date Of Birth:</span> {formatDate(memberApplication?.dateOfBirth) || "N/A"}</p>
            <p><span className="text-slate-400">Gender:</span> {memberApplication?.gender || "N/A"}</p>
            <p><span className="text-slate-400">Major:</span> {memberApplication?.major || "N/A"}</p>
            <p><span className="text-slate-400">Student ID:</span> {memberApplication?.studentId || "N/A"}</p>
            <p><span className="text-slate-400">Applied At:</span> {formatDate(memberApplication?.appliedAt) || "N/A"}</p>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-700 bg-slate-900/65 p-4">
          ...
          <div className="space-y-2 text-sm">
            <p><span className="text-slate-400">Status:</span> {cvStatus || "N/A"}</p>
            <p><span className="text-slate-400">Comment:</span> {memberApplication?.cvReviewComment || "N/A"}</p>
            <p><span className="text-slate-400">Reviewed At:</span> {formatDate(memberApplication?.cvReviewedAt) || "N/A"}</p>
            <p><span className="text-slate-400">Reviewed By:</span> {memberApplication?.cvReviewer?.fullname || "N/A"}</p>
          </div>
        </section>

        <section className="rounded-2xl border border-slate-700 bg-slate-900/65 p-4">
          <h2 className="mb-3 text-lg font-bold">Interview Round (By Department)</h2>

          {deptApplications.length === 0 ? (
            <p className="text-sm text-slate-300">No department interview records yet.</p>
          ) : (
            <div className="space-y-3">
              {deptApplications.map((deptApp) => (
                <div key={deptApp.id} className="rounded-xl border border-slate-700 bg-slate-950/40 p-3">
                  <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                    <h3 className="font-semibold text-slate-100">
                      {deptApp.department?.name || `Department #${deptApp.departmentId}`}
                    </h3>
                    <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${badgeClass(deptApp.interviewStatus)}`}>
                      {normalizeStatus(deptApp.interviewStatus) || "PENDING"}
                    </span>
                  </div>
                  <p className="text-sm"><span className="text-slate-400">Priority:</span> {deptApp.priority ?? "N/A"}</p>
                  <p className="text-sm"><span className="text-slate-400">Interviewer:</span> {deptApp.interviewer?.fullname || "N/A"}</p>
                  <p className="text-sm"><span className="text-slate-400">Interviewed At:</span> {formatDate(deptApp.interviewedAt) || "N/A"}</p>
                  <p className="text-sm"><span className="text-slate-400">Comment:</span> {deptApp.interviewComment || "N/A"}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <section className="rounded-2xl border border-slate-700 bg-slate-900/65 p-4">
          ...
          <div className="space-y-2 text-sm">
            <p><span className="text-slate-400">Status:</span> {finalStatus || "N/A"}</p>
            <p><span className="text-slate-400">Comment:</span> {memberApplication?.finalReviewComment || "N/A"}</p>
            <p><span className="text-slate-400">Reviewed At:</span> {formatDate(memberApplication?.finalReviewedAt) || "N/A"}</p>
            <p><span className="text-slate-400">Reviewed By:</span> {memberApplication?.finalReviewer?.fullname || "N/A"}</p>
          </div>
        </section>
      </div>
    </div>
