...
const ProfilePage = () => {
  ...
  const clubInfo = [
    ...
    { label: "Generation", value: user?.generation || "Not provided" },
    {
      label: "Joined At",
    ...
    {
      label: "Role",
      value: user?.userPosition?.[0]?.position?.systemRole
        ? formatUppercaseToCapitalized(user.userPosition[0].position.systemRole)
        : "Not provided",
    },
    { label: "Position", value: user?.userPosition?.[0]?.position?.title || "Not provided" },
  ];
  ...
  const completenessFields = [
    ...
    user?.userPosition?.[0]?.position?.title,
  ];
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
              <div className="relative">
                <img
                  src={user.avatarUrl || "https://via.placeholder.com/160?text=Profile"}
                  alt="Avatar"
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-slate-300 md:text-base">Your personal and club profile information.</p>
                <div className="pt-1">
                  <span className="rounded-full border border-emerald-500/30 bg-emerald-500/15 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-emerald-200">
                    Status: {user?.status ? formatUppercaseToCapitalized(user.status) : "Not provided"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className={`${cardClassName} overflow-hidden`}>
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  <p className="mt-2 text-lg font-semibold text-white">{profileCompletion}%</p>
                </div>

                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Completed Fields</p>
                    <p className="mt-2 font-semibold text-white">
                      {completedFields}/{totalFields}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Missing Fields</p>
                    <p className="mt-2 font-semibold text-white">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Bio Length</p>
                  <p className="mt-2 font-semibold text-white">
                    {user?.bio ? `${user.bio.trim().length} characters` : "Not provided"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <section className={cardClassName}>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {personalInfo.map((field) => (
                  <div key={field.label} className="rounded-2xl border border-white/10 bg-white/5 p-4">
